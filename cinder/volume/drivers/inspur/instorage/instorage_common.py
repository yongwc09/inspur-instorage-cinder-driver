...
instorage_mcs_opts = [
    ...
    cfg.IntOpt('instorage_mcs_vol_rsize',
               default=2,
               # min=-1, max=100, #TODO parameter not support before Kilo
    ...
    cfg.IntOpt('instorage_mcs_localcopy_timeout',
               default=120,
               # min=1, max=600, #TODO parameter not support before Kilo
...
class InStorageMCSCommonDriver(san.SanDriver, driver.VolumeDriver):
    ...
    def _run_ssh(self, cmd_list, check_exit_code=True, attempts=1):
        ...
        if not self.sshpool:
            try:
                ...
                LOG.warning(_('Unable to use san_ip to create SSHPool. Now '
                              'attempting to use instorage_san_secondary_ip '
                              'to create SSHPool.'))
                if self._switch_ip():
                    self.sshpool = self._set_up_sshpool(self.active_ip)
                else:
                    LOG.error(_('Unable to create SSHPool using san_ip '
                                'and not able to use '
        ...
        try:
            ...
            try:
                if self._switch_ip():
                    LOG.warning(_("Unable to execute SSH command with "
            ...
            with sshpool.item() as ssh:
                while attempts > 0:
                    ...
                    try:
                        ...
                        LOG.exception(_('Error has occurred: %s'),
                                      six.text_type(e))
                        last_exception = e
    ...
    def _extend_volume_op(self, volume, new_size, old_size=None):
        ...
        if rel_info:
            LOG.warning(_('_extend_volume_op: Extending a volume with '
                          'remote copy is not recommended.'))
            try:
                tgt_vol = (instorage_const.REPLICA_AUX_VOL_PREFIX +
                           volume['name'])
    ...
    def _failback_replica_volumes(self, ctxt, rep_volumes):
        ...
        for volume in rep_volumes:
            rep_type = self._get_volume_replicated_type(ctxt, volume)
            replica_obj = self._get_replica_obj(rep_type)
            tgt_volume = instorage_const.REPLICA_AUX_VOL_PREFIX + \
                volume['name']
...
class InStorageAssistant(object):
    ...
    def _wait_for_a_condition(self, testmethod, timeout=None,
        ...
        def _inner():
            try:
                ...
                if raise_exception:
                    LOG.exception(_("_wait_for_a_condition: %s"
                                    " execution failed."),
                                  testmethod.__name__)
    ...
    def run_consistgrp_snapshots(self, lc_consistgrp, snapshots, state,
        ...
        try:
            ...
            # Release cg
            self.delete_lc_consistgrp(lc_consistgrp)
            LOG.error(_("Failed to create CGSnapshot. "
                        "Exception: %s."), err)
    ...
    def get_relationship_info(self, volume_name):
        ...
        if not vol_attrs or not vol_attrs['RC_name']:
            LOG.info(_("Unable to get remote copy information for "
    ...
    def delete_rc_volume(self, volume_name, target_vol=False):
        ...
        try:
            ...
            msg = (_('Unable to delete the volume for '
                   ...
                   {'vol': vol_name, 'err': six.text_type(e)})
            LOG.exception(msg)
            raise exception.VolumeDriverException(message=msg)
